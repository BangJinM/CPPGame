...
void GameEngine::AssetLoader::Finalize()
{
    m_strSearchPath.clear();
}

void GameEngine::AssetLoader::Tick()
{

}

bool GameEngine::AssetLoader::AddSearchPath(const char *path)
{
...
bool GameEngine::AssetLoader::RemoveSearchPath(const char *path)
{
    std::vector<std::string>::iterator src = m_strSearchPath.begin();
...
bool GameEngine::AssetLoader::FileExists(const char *filePath)
{
...
GameEngine::AssetLoader::AssetFilePtr GameEngine::AssetLoader::OpenFile(const char* name, AssetOpenMode mode)
{
    FILE *fp = nullptr;
...
GameEngine::Buffer GameEngine::AssetLoader::SyncOpenAndReadText(const char *filePath)
{
...
GameEngine::Buffer GameEngine::AssetLoader::SyncOpenAndReadBinary(const char *filePath)
{
    AssetFilePtr fp = OpenFile(filePath, MY_OPEN_BINARY);
    Buffer* pBuff = nullptr;
...
void GameEngine::AssetLoader::CloseFile(AssetFilePtr& fp)
{
    fclose((FILE*)fp);
    fp = nullptr;
}

size_t GameEngine::AssetLoader::GetSize(const AssetFilePtr& fp)
{
...
size_t GameEngine::AssetLoader::SyncRead(const AssetFilePtr& fp, Buffer& buf)
{
    size_t sz;
...
int32_t GameEngine::AssetLoader::Seek(AssetFilePtr fp, long offset, AssetSeekBase where)
{
